    ...
    if 'qa' in st.session_state:
        ...
        if submit_button and question:
            with st.spinner("Generating answer..."):
                try:
                    # Debugging Step 1: Check the generated Cypher query
                    res = st.session_state['qa'].invoke({"query": question})
                    cypher_query = res.get('result', '')  # Safely extract result
                    
                    # Debugging Step 2: Print generated query for inspection
                    st.write(f"**Generated Cypher Query:**\n{cypher_query}")
                    
                    # Debugging Step 3: Ensure it's not an empty query
                    if not cypher_query.strip():
                        st.error("The generated Cypher query is empty or incomplete.")
                    else:
                        st.write("\n**Answer:**\n" + cypher_query)
                except Exception as e:
                    # Debugging Step 4: Catch any errors during the execution
                    st.error(f"Failed to generate or execute the query: {e}")
